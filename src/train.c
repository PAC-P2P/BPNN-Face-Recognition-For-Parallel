...
void backprop_face_choose(IMAGELIST *trainlist,int id,BPNN **net, int epochs, int savedelta, double sume[], map_t *map_user)
{
    IMAGE *iimage;
    int i, imagesize, train_n, epoch;
    double out_err, hid_err, sumerr;
    ...
    *net = bpnn_create(imagesize, userNum, userNum);
    ...
    for(epoch = 1;epoch <= SELETE; epoch++){
        sumerr = 0;
        for (i = 0; i < train_n; i++) {
            /** Set up input units on net with image i **/
            // 用训练集中图片i来设置输入层单元
